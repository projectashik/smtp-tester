...
import { ArrowLeft, CheckCircle, Mail, Server, Shield, Zap } from 'lucide-react';
...
const providers = {
  gmail: {
    ...
    port: 587,
    security: 'STARTTLS',
    description: 'Test your Gmail SMTP configuration with our professional testing tool. Verify Gmail email delivery, authentication, and troubleshoot common Gmail SMTP issues.',
    features: [
  ...
  outlook: {
    ...
    port: 587,
    security: 'STARTTLS',
    description: 'Test your Outlook/Hotmail SMTP server configuration. Validate Microsoft email service connectivity and troubleshoot Outlook SMTP authentication issues.',
    features: [
  ...
  yahoo: {
    ...
    port: 587,
    security: 'STARTTLS',
    description: 'Test Yahoo Mail SMTP configuration and validate email delivery. Troubleshoot Yahoo SMTP authentication and connection issues with detailed diagnostics.',
    features: [
  ...
  sendgrid: {
    ...
    port: 587,
    security: 'STARTTLS',
    description: 'Test SendGrid SMTP API configuration for transactional email delivery. Validate SendGrid authentication and troubleshoot email sending issues.',
    features: [
  ...
  mailgun: {
    ...
    port: 587,
    security: 'STARTTLS',
    description: 'Test Mailgun SMTP configuration for reliable email delivery. Validate Mailgun authentication and troubleshoot transactional email issues.',
    features: [
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const provider = providers[params.provider as ProviderKey];
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      images: [
        `/api/og?title=${encodeURIComponent(title)}&description=${encodeURIComponent(description)}&provider=${encodeURIComponent(provider.name)}&type=provider`
      ],
    },
    twitter: {
      title,
...
export default function ProviderTestPage({ params }: PageProps) {
  const provider = providers[params.provider as ProviderKey];
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="text-gray-900 font-mono">{provider.host}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium text-gray-700">Port:</span>
                <span className="text-gray-900 font-mono">{provider.port}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-medium text-gray-700">Security:</span>
                <span className="text-gray-900 font-mono">{provider.security}</span>
              </div>
              <div className="flex justify-between">
        ...
        <div className="text-center bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl p-8 text-white">
          ...
          <p className="text-blue-100 mb-6 max-w-2xl mx-auto">
            Use our professional SMTP testing tool to validate your {provider.name} email server configuration, 
            troubleshoot issues, and ensure reliable email delivery.
          </p>
          <Link
            href="/"
